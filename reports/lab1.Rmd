...
#Preparando os Dados
A primeira parte foi construir um data frame com os dados que foram obtidos do [IMDB](http://www.imbd.com/). <br />
...
>> Não vejo problema nenhum em usar as tags de html pra formatar teu relatório, mas tenta usar as opções do markdown sempre que possível; por exemplo: ao invés de <b>Texto em negrito</b> usar  \**Texto em negrito\**
...
```{r}
scandal = shondaland %>% 
    filter(series_name %in% c("Scandal"))
...
basic_statistic = shondaland %>%
    ...
    summarize(variance = var(user_rating), max =max(user_rating), min = min(user_rating), average = mean(user_rating), sd = sd(user_rating))

general_statistic <- shondaland %>%
    group_by(series_name) %>%
     summarize(variance = var(user_rating), max =max(user_rating), min = min(user_rating), average = mean(user_rating), sd = sd(user_rating))
...
Podemos mostrar isso com a ajuda de valores estatísticos como a variancia e desvião padrão, esses valores nos ajudam a compreender quão grande é a diferença dos valores reais relação a média esperada. Nesta análise usaremos o desvio padrão.
```{r}
shondaland_sd_general = ggplot(data = general_statistic, 
                               aes(x = series_name, y = sd, fill = series_name)) +
    geom_bar(stat = "identity") +
    labs(x = "",
         y = "Desvio Padrão",
         color = "Série") +
    ggtitle("Desvio padrão na nota dos episódios")
...
grid.arrange(shondaland_sd_general, shondaland_sd)
```
Embora ambos os valores sejam pequenos, o desvio padrão no caso de <b>Scandal</b> é maior.
