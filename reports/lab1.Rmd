...
scandal_nah <- shonda_nah %>% filter(series_name %in% c("How to Get Away with Murder"))

ggplot(scandal_nah, aes(season_ep, nah, col=season, fill=season)) + 
        geom_bar(width=0.4, stat="identity") +
        geom_text(aes(label=sprintf("%0.2f", round(nah, digits = 2))),hjust=0.5, vjust=-0.5, size= 3,  color="#000000") +
